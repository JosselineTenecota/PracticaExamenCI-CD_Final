...
// COMPONENTE ADICIONAL: Configuración de retardo simulado en el arranque (en segundos)
const STARTUP_DELAY = parseInt(process.env.STARTUP_DELAY_SECONDS || '0', 10) * 1000;
...
function createApp() {
  ...
  app.get('/health', (req, res) => {
    // 1. Verificación de arranque lento (Readiness probe)
    const uptime = Date.now() - startTime;
    if (uptime < STARTUP_DELAY) {
      return res.status(503).json({
        status: 'not_ready',
        reason: `Inicializando aplicación... (${Math.ceil((STARTUP_DELAY - uptime) / 1000)}s restantes)`
      });
    }

    // 2. Verificación de fallos simulados o acceso a BD
